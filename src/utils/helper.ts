...
class Helper {
        ...
        if (err) {
          ...
          let mailOptions = {
            ...
            html: data
              ...
              .replace(/\%SITE_EMAIL%/g, config.get("admin.email"))
  ...
  async mailStaticTemplates(type: string, userData: any) {
    if (userData.email) {
      ...
      switch (type) {
        case "reset-password":
          let reset_link: string = `${config.get("siteUrl")}/reset-password/${
            userData.resetToken
          }`;
...
export function encodeSecretToken(randomBytes, paymentId) {
  return Buffer.from(`${randomBytes}:${paymentId}`).toString("base64");
}

export function decodeSecretToken(secretToken) {
